        ...
        <tbody>
          {data?.map(
            (item: FieldValues, index: number) =>
              (item.category == Number(selectRef.current?.value) || Number(selectRef.current?.value) === -1) && (
                <tr>
                  <td>{item.description}</td>
                  <td>{item.amount}</td>
                  <td>{categories[item.category]}</td>
                  <td>
                    <button onClick={() => onDelete(index)} className="btn btn-outline-danger">Delete</button>
                  </td>
                </tr>
              ),
          )}
